...
parser.add_argument(
    ...
    action="store",
    dest="candidates_dir",
    help="Location of html candidates.",
)
...
for ln in lnames:
    ...
    if not ln.startswith("."):
        ...
        for n in names:
            ...
            if n.startswith("candidate"):
                ...
                if match:
                    text = match.group()
                    if (
                        u"Verb</h3>" in text and u"-unud" not in text
                    ):  # ESTONIAN HACK TO GET BETTER EXAMPLE...
                        try:
                            data = pd.read_html(text)
                            if len(data) >= 1:
                                data = pd.concat(data)
        ...
        for shape, table in adj_shape_example_dict.items():
            ...
            table.to_csv(
                out_dir1 + ln + "/ADJ_" + count + "_" + shape + "_example.csv",
                encoding="utf-8",
                index=False,
            )
            table.to_csv(
                out_dir2 + ln + "/ADJ_" + count + "_" + shape + "_example.csv",
                encoding="utf-8",
                index=False,
            )
        ...
        for shape, table in n_shape_example_dict.items():
            ...
            shape = str(shape).replace(" ", "")
            table.to_csv(
                out_dir1 + ln + "/N_" + count + "_" + shape + "_example.csv",
                encoding="utf-8",
                index=False,
            )
            table.to_csv(
                out_dir2 + ln + "/N_" + count + "_" + shape + "_example.csv",
                encoding="utf-8",
                index=False,
            )
        ...
        for shape, table in pn_shape_example_dict.items():
            ...
            shape = str(shape).replace(" ", "")
            table.to_csv(
                out_dir1 + ln + "/PN_" + count + "_" + shape + "_example.csv",
                encoding="utf-8",
                index=False,
            )
            table.to_csv(
                out_dir2 + ln + "/PN_" + count + "_" + shape + "_example.csv",
                encoding="utf-8",
                index=False,
            )
        ...
        for shape, table in v_shape_example_dict.items():
            ...
            shape = str(shape).replace(" ", "")
            table.to_csv(
                out_dir1 + ln + "/V_" + count + "_" + shape + "_example.csv",
                encoding="utf-8",
                index=False,
            )
            table.to_csv(
                out_dir2 + ln + "/V_" + count + "_" + shape + "_example.csv",
                encoding="utf-8",
                index=False,
            )
